...
import { FileText, Image, Tag, Star, FileEdit, Upload, X, AlertCircle, Database,Plus  } from 'lucide-react';
...
export default function CreateJSONProductPage() {
  ...
  const fetchProduct = useCallback(async () => {
    try {
      ...
      const colorImages = (product.colors || []).map((c, idx) => ({
        ...
        preview: c.image || '',
        color: c.hex || '#000000',
        name: c.name || `Màu ${idx + 1}`, // All colors from 1 to 4
      }));
  ...
  const handleImageUrlChange = (index, url) => {
    ...
    if (!newImages[index]) {
      newImages[index] = { src: '', name: '', color: '#000000' };
    }
  ...
  const handleAddImageInput = () => {
    setImages([...images, { src: '', name: '', color: '#000000', preview: '' }]);
  };

  const handleColorChange = (index, color) => {
    console.log('Changing color for index:', index, 'to:', color); // Debug log
    console.log('Current images state:', images); // Debug log
    setImages((prev) => {
      const newImages = [...prev];
      console.log('Before update - newImages:', newImages); // Debug log
      if (newImages[index]) {
        newImages[index] = { ...newImages[index], color: color };
        console.log('After update - newImages[index]:', newImages[index]); // Debug log
      } else {
        console.log('Index not found:', index); // Debug log
      }
      console.log('Final newImages:', newImages); // Debug log
      return newImages;
    });
  };
    ...
    try {
      ...
      if (uploadedUrls.length !== validImages.length) {
        ...
        const productData = {
          ...
          colors: validImages.map((img, idx) => ({ // All 4 images in colors array
            name: img.name || `Màu ${idx + 1}`, // Name from 1 to 4
            hex: img.color || '#000000',
            image: img.src,
          })),
        };
              ...
              <div className={styles.imageInputSection}>
                ...
                <div className={styles.mainImageRow}>
                  ...
                  <div className={styles.colorInputGroup}>
                    <input
                      type="color"
                      value={images[0]?.color || '#000000'}
                      onChange={(e) => {
                        console.log('Color picker changed for index 0:', e.target.value); // Debug log
                    ...
                    <input
                      ...
                      value={images[0]?.color || '#000000'}
                      onChange={(e) => {
                        console.log('Hex input changed:', e.target.value); // Debug log
                        handleColorChange(0, e.target.value);
                      }}
                  ...
                  <div key={index + 1} className={styles.imageInputRow}>
                    ...
                    <div className={styles.colorInputGroup}>
                      ...
                      <input
                        ...
                        value={img.color || '#000000'}
                        onChange={(e) => {
                          console.log('Hex input changed for index:', index + 1, 'value:', e.target.value); // Debug log
